...
def get_embedding_for_sequence_token(args, sequence_filename, mer):
    ...
    for index, dataitem in enumerate(dataloader):
        ...
        token_keys = df_raw["text"][index].split(" ")
        all_token_keys.append(token_keys)
        ...
        with torch.no_grad():
            ...
            token_embeddings_last_layers = outputs[4].to("cpu")
            all_token_embeddings_sequence.append(token_embeddings_last_layers[:, 1:].tolist())

    all_token_embeddings_sequence = np.array(all_token_embeddings_sequence)
    all_token_keys = np.array(all_token_keys)
    return all_token_embeddings_sequence, all_token_keys
